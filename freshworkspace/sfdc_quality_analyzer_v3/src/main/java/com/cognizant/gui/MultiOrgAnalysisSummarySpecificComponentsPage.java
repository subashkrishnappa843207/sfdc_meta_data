...
public class MultiOrgAnalysisSummarySpecificComponentsPage implements Initializable {
	private static final Logger log = Logger.getLogger(MultiOrgAnalysisSummarySpecificComponentsPage.class);
    ...
    private void RemoveComponentFromListView() throws IOException {
	      ...
	      log.info("Removed Items = "+removedItem);
	      if (removedItem != null) {
    ...
    private void FindImpactedComponents() throws IOException, JSONException, ParseException {
    	...
    	if(SalesforceSelectedComponentListView.getItems().size() == 0) {
    		Alert SelectedComponentErrorAlert = new Alert(AlertType.ERROR);
    		SelectedComponentErrorAlert.setHeaderText("Salesforce Component selection Error");
    		SelectedComponentErrorAlert.setContentText("Please select the Salesforce Component before proceed!!!");
    		SelectedComponentErrorAlert.showAndWait();
    	} else {
        	...
        	ArrayList<String> ArrayListMultiOrgSource = new ArrayList<String>(); 
        	ArrayList<String> ArrayListMultiOrgTarget = new ArrayList<String>(); 
    		...
    		for(int i=0;i<jsonRecordsArrayMultiOrgSource.length();i++) {
    			JSONObject jsonRecordsMultiOrgSource = new JSONObject(jsonRecordsArrayMultiOrgSource.get(i).toString());
    		...
    		for(int i=0;i<jsonRecordsArrayMultiOrgTarget.length();i++) {
    			JSONObject jsonRecordsMultiOrgTarget = new JSONObject(jsonRecordsArrayMultiOrgTarget.get(i).toString());
    		...
    		for(int i = 0 ; i < selectedSObjects.size() ; i++) {
				...
				totalCount = totalCount + oListMissing.size();
    		...
    		if(selectedSObjects.size() > 0) {
    			...
    			RecordCount.setText(String.valueOf(selectedSObjects.size()));
